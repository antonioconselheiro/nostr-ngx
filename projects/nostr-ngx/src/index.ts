import { WindowNostr } from 'nostr-tools/nip07';

export * from './lib/configs/abstract-browser-storage';
export * from './lib/configs/accounts-local.storage';
export * from './lib/configs/nostr-config.interface';
export * from './lib/configs/nostr-local-config.interface';
export * from './lib/configs/nostr-session-config.interface';
export * from './lib/configs/nostr-user-relays.interface';
export * from './lib/configs/nostr.config';
export * from './lib/configs/nostr.module';
export * from './lib/configs/profile-session.storage';
export * from './lib/domain/account/account-authenticable.interface';
export * from './lib/domain/account/account-calculated.interface';
export * from './lib/domain/account/account-complete.interface';
export * from './lib/domain/account/account-essential.interface';
export * from './lib/domain/account/account-nip05-detail.type';
export * from './lib/domain/account/account-pointable.interface';
export * from './lib/domain/account/account-raw.interface';
export * from './lib/domain/account/account-state.type';
export * from './lib/domain/account/compose/account-openable.type';
export * from './lib/domain/account/compose/account-renderable.type';
export * from './lib/domain/account/compose/account-session.type';
export * from './lib/domain/account/compose/account.interface';
export * from './lib/domain/account/nostr-metadata.type';
export * from './lib/domain/base64-string.type';
export * from './lib/domain/event/nostr-event-relation.interface';
export * from './lib/domain/event/nostr-event-tags.array';
export * from './lib/domain/event/nostr-event-with-relays.interface';
export * from './lib/domain/event/nostr-event.collection';
export * from './lib/domain/event/nostr-event.interface';
export * from './lib/domain/event/nostr-nip1-event.interface';
export * from './lib/domain/event/nostr-raw-event.interface';
export * from './lib/domain/event/nostr-strict-event.type';
export * from './lib/domain/event/nostr-strict-tags.array';
export * from './lib/domain/event/primitive/hex-string.type';
export * from './lib/domain/event/primitive/stringified-event.type';
export * from './lib/domain/event/primitive/stringified.type';
export * from './lib/domain/event/relay-domain-string.type';
export * from './lib/domain/event/strict-tag-kinds.type';
export * from './lib/domain/event/strict-tag.array';
export * from './lib/domain/event/strict-tag.type-map';
export * from './lib/domain/event/tags/tag-amount-millisats.type';
export * from './lib/domain/event/tags/tag-bolt11.type';
export * from './lib/domain/event/tags/tag-common.type';
export * from './lib/domain/event/tags/tag-lnurl.type';
export * from './lib/domain/event/tags/tag-note.type';
export * from './lib/domain/event/tags/tag-pointer-related.type';
export * from './lib/domain/event/tags/tag-pointer.type';
export * from './lib/domain/event/tags/tag-reaction.type';
export * from './lib/domain/event/tags/tag-reference.type';
export * from './lib/domain/event/tags/tag-related.type';
export * from './lib/domain/event/tags/tag-relay-list.type';
export * from './lib/domain/event/tags/tag-zap-receipt.type';
export * from './lib/domain/event/tags/tag-zap-request.type';
export * from './lib/domain/nostr-public-user.interface';
export * from './lib/domain/profile-pointer-sanitized.interface';
export * from './lib/domain/relay-list.event-kind';
export * from './lib/domain/request/auth.request';
export * from './lib/domain/request/close.request';
export * from './lib/domain/request/count.request';
export * from './lib/domain/request/event.request';
export * from './lib/domain/request/nostr.request';
export * from './lib/domain/request/req.request';
export * from './lib/domain/resultset/auth.resultset';
export * from './lib/domain/resultset/closed.resultset';
export * from './lib/domain/resultset/count.resultset';
export * from './lib/domain/resultset/eose.resultset';
export * from './lib/domain/resultset/event-with-relays.resultset';
export * from './lib/domain/resultset/event.resultset';
export * from './lib/domain/resultset/nostr.resultset';
export * from './lib/domain/resultset/notice.resultset';
export * from './lib/domain/resultset/ok.resultset';
export * from './lib/domain/resultset/resultset.map';
export * from './lib/domain/symbol/index-not-found.const';
export * from './lib/domain/view-model/context/note-content-event.segment';
export * from './lib/domain/view-model/context/note-content-hashtag.segment';
export * from './lib/domain/view-model/context/note-content-image.segment';
export * from './lib/domain/view-model/context/note-content-link.segment';
export * from './lib/domain/view-model/context/note-content-note.segment';
export * from './lib/domain/view-model/context/note-content-nprofile.segment';
export * from './lib/domain/view-model/context/note-content-npub.segment';
export * from './lib/domain/view-model/context/note-content-relay.segment';
export * from './lib/domain/view-model/context/note-content-segment.type';
export * from './lib/domain/view-model/context/note-content-segment.view-model';
export * from './lib/domain/view-model/context/note-content-summarized.segment';
export * from './lib/domain/view-model/context/note-content-text.segment';
export * from './lib/domain/view-model/context/note-content-video.segment';
export * from './lib/domain/view-model/context/note-content.view-model';
export * from './lib/domain/view-model/eager-note.view-model';
export * from './lib/domain/view-model/event-relation.type';
export * from './lib/domain/view-model/feed.view-model';
export * from './lib/domain/view-model/lazy-note.view-model';
export * from './lib/domain/view-model/nostr-event-id.view-model';
export * from './lib/domain/view-model/nostr-event.view-model';
export * from './lib/domain/view-model/nostr-view-model.set';
export * from './lib/domain/view-model/note-reflection.view-model';
export * from './lib/domain/view-model/note.view-model';
export * from './lib/domain/view-model/reaction.view-model';
export * from './lib/domain/view-model/related-content.view-model';
export * from './lib/domain/view-model/relay-domain.type';
export * from './lib/domain/view-model/repost-note.view-model';
export * from './lib/domain/view-model/simple-text-note.view-model';
export * from './lib/domain/view-model/zap.view-model';
export * from './lib/exceptions/no-credentials-found.error';
export * from './lib/exceptions/not-supported-by-signer.error';
export * from './lib/exceptions/signer-not-found.error';
export * from './lib/idb-cache/idb-ncache.module';
export * from './lib/idb-cache/idb-nostr-event.interface';
export * from './lib/idb-cache/idb-nostr-profile.interface';
export * from './lib/idb-cache/idb.cache';
export * from './lib/idb-cache/idb.event-cache';
export * from './lib/idb-cache/idb.profile-cache';
export * from './lib/in-memory/in-memory.cache';
export * from './lib/in-memory/in-memory.event-cache';
export * from './lib/in-memory/in-memory.module';
export * from './lib/in-memory/in-memory.profile-cache';
export * from './lib/injection-token/nostr-cache.interface';
export * from './lib/injection-token/nostr-cache.token';
export * from './lib/injection-token/nostr-config.token';
export * from './lib/injection-token/nostr-profile-cache.interface';
export * from './lib/injection-token/relay-router.token';
export * from './lib/nostr-media/file-manager.service';
export * from './lib/nostr-media/file-processing.interface';
export * from './lib/nostr-media/file-sending.interface';
export * from './lib/nostr-media/file-upload-completed.interface';
export * from './lib/nostr-media/file-upload.type';
export * from './lib/nostr-media/nostr-media.module';
export * from './lib/nostr-utils/nostr-utils.module';
export * from './lib/nostr-utils/nostr.converter';
export * from './lib/nostr-utils/nostr.guard';
export * from './lib/nostr-utils/nsec.crypto';
export * from './lib/nostr-utils/relay.converter';
export * from './lib/pool/default.router-matcher';
export * from './lib/pool/event-router-matcher.interface';
export * from './lib/pool/nostr-cache.service';
export * from './lib/pool/nostr-event.iterable';
export * from './lib/pool/nostr-filter.interface';
export * from './lib/pool/nostr-pool';
export * from './lib/pool/nostr-relay';
export * from './lib/pool/pool-request.options';
export * from './lib/pool/pool-router.options';
export * from './lib/pool/pool.module';
export * from './lib/pool/relay-local-config.service';
export * from './lib/pool/relay-router.service';
export * from './lib/pool/router-matcher.interface';
export * from './lib/profile/account-manager.service';
export * from './lib/profile/account.factory';
export * from './lib/profile/account.guard';
export * from './lib/profile/current-profile.observable';
export * from './lib/profile/idb-account-cache.interface';
export * from './lib/profile/nip05-resultset.interface';
export * from './lib/profile/nip05.proxy';
export * from './lib/profile/nostr.signer';
export * from './lib/profile/profile-event.factory';
export * from './lib/profile/profile.cache';
export * from './lib/profile/profile.module';
export * from './lib/profile/profile.nostr';
export * from './lib/profile/profile.proxy';
export * from './lib/tools/unix-date.fn';
export * from './lib/view-model-mapper/account-view-model.proxy';
export * from './lib/view-model-mapper/feed.mapper';
export * from './lib/view-model-mapper/note-content.mapper';
export * from './lib/view-model-mapper/reaction.mapper';
export * from './lib/view-model-mapper/repost.mapper';
export * from './lib/view-model-mapper/simple-text.mapper';
export * from './lib/view-model-mapper/single-view-model.mapper';
export * from './lib/view-model-mapper/tag.helper';
export * from './lib/view-model-mapper/view-model-mapper.module';
export * from './lib/view-model-mapper/view-model-patch.mapper';
export * from './lib/view-model-mapper/view-model.guard';
export * from './lib/view-model-mapper/view-model.mapper';
export * from './lib/view-model-mapper/zap.mapper';

declare global {
  interface Window {
    nostr?: WindowNostr;
  }
}
